        ...
        if (/pasar|toko/i.test(command)) {
           ...
           /* case 'buy':
            ...
            case 'jual': 
                switch (_type) {                  
                     case 'bul':
                        if (global.db.data.users[m.sender].bul >= count * 1) {
                            global.db.data.users[m.sender].money += Sbul * count
                            global.db.data.users[m.sender].bul -= count * 1
                            conn.reply(m.chat, `Sukses Menjual ${count} Bul Dengan Harga ${Sbul * count} Money `.trim(), m)
                     } else conn.reply(m.chat, `Bul Kamu Tidak Cukup`.trim(), m)
                     break
                     case 'banteng':
            ...
            switch (type) { 
                       ...
                       case 'banteng':
                        ...
                        if (global.db.data.users[m.sender].panda >= count * 1) {
                            global.db.data.users[m.sender].money += Spanda * count
                            global.db.data.users[m.sender].panda -= count * 1
                            conn.reply(m.chat, `Sukses Menjual ${count} Panda Dengan Harga ${panda * count} Money `.trim(), m)
                        } else conn.reply(m.chat, `Panda Kamu Tidak Cukup`.trim(), m)
                        break
                        case 'buaya':
                        if (global.db.data.users[m.sender].buaya >= count * 1) {
