...
async function handler(m, { conn, args }) {
    ...
    try {
        ...
        let count = (args[0] && number(parseInt(args[0])) ? Math.max(parseInt(args[0]), 1) : /all/i.test(args[0]) ? Math.floor(parseInt(user.chip)) : 1) * 1
        if ((user.chip * 1) < count) return m.reply('♋ Chip kamu tidak cukup!!')
    ...
    try {
        if (/^(✔️|y(es|a))?$/i.test(txt)) {
            ...
            if (Bot < Kamu) {
                ...
                status = 'Seri'
                user.chip += (Math.floor(count / 1)) * 1
            }
            m.reply(`
| *PLAYERS* | *POINT* |
*🤖 BOT:*      ${Bot}
...
Kamu *${status}*, kamu ${status == 'Menang' ? `Mendapatkan *+${count * 2}*` : status == 'Kalah' ? `Kehilangan *-${count * 1}*` : `Mendapatkan *+${Math.floor(count / 1)}*`} Chip ♋
    `.trim())
            clearTimeout(timeout)
            delete confirm[m.sender]
            return !0
        ...
        clearTimeout(timeout)
        delete confirm[m.sender]
        if (chipDulu > (user.chip * 1)) user.chip = chipDulu * 1
...
handler.help = ['csn [jumlah]']
handler.tags = ['rpg']
handler.command = /^(csn)$/i
